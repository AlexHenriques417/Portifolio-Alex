...
async function loadSkills() {
  try {
    ...
    if (data && data.length > 0) {
      ...
      if (softContainer) {
        softContainer.innerHTML = softSkills.map(s => `<span class="soft-tag">${s.name}</span>`).join('');
      }
      ...
      if (container) {
        container.innerHTML = data.map(p => `
          <div class="project-card reveal">
            ${p.image_url ? `<img src="${p.image_url}" class="project-image" alt="${p.title}">` : '<div class="project-image" style="display: flex; align-items: center; justify-content: center; background: var(--surface);">🚀 Projeto</div>'}
